...
| Add EAS config draft | needs_quality_check | See `eas.json` and `docs/eas-build-plan.md`. First internal iOS build installed but opened to a blank screen; build `2` is prepared with env correction and error fallback. |
